...
Original file is located at
    https://colab.research.google.com/drive/1oUl5jodX9OvP9Tc79L1fJMLhAKxX8qN2
...
def gs1_error(n): 
    true_vals = np.array([1,1])
    approxVals = np.array(gs1_method(n))
    return np.linalg.norm(true_vals-approxVals)
gs1_error(3)
...
def gs2_error(n): 
    true_vals = np.array([2,1])
    approxVals = np.array(gs2_method(n))
    return np.linalg.norm(true_vals-approxVals)
...
def gs3_iteration(x,y,z): 
    ...
    new_y = z - (new_x/4) + (51/2)
    new_z = (new_x/2) + (new_y/2) - (45/2)
